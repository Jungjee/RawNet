...
def res_dense_block(inputs, nb_node, initializer, regularizer, drop_ratio, name):
	x = Dense(nb_node,
		kernel_initializer = initializer,
		kernel_regularizer = regularizers.l2(regularizer),
		name = name + 'dense_1')(inputs)
	#x = BatchNormalization(axis=-1, name='e2e_BN_%d'%i)(x)
	x = Dropout(drop_ratio, name= name + 'Dropout_1')(x)
	x = LeakyReLU(name = name + 'LRelu_1')(x)
	...
	x = Dense(nb_node,
		...
		name = name + 'dense_2')(x)
	#x = BatchNormalization(axis=-1, name='e2e_BN_%d'%i)(x)
	#x = Dropout(drop_ratio, name= name + 'Dropout_2')(x)
	x = Add()([inputs, x])
	x = LeakyReLU(name = name + 'LRelu_2')(x)
	#x = Activation('relu', name = name + 'Relu_2')(x)

	return x

def res_dense_block_fullpre(inputs, nb_node, initializer, regularizer, drop_ratio, name):
	#x = BatchNormalization(axis=-1, name='e2e_BN_%d'%i)(x)
	#x = Dropout(drop_ratio, name= name + 'Dropout_1')(x)
	x = Dropout(drop_ratio, name= name + 'Dropout_1')(inputs)
	x = LeakyReLU(name = name + 'LRelu_1')(x)
	#x = Activation('relu', name = name + 'Relu_1')(x)
...
def get_model(argDic):
	...
	x = Dense(argDic['nb_dense_node'][0],
		kernel_initializer = argDic['initializer'],
		kernel_regularizer = regularizers.l2(argDic['wd']),
		name = 'e2e_dense_1')(x)
	#x = Dropout(argDic['drop_ratio'], name= 'e2e_Dropout_1')(x)
	#x = Activation('relu', name = 'e2e_Relu_1')(x)
	#x = LeakyReLU(name = 'e2e_LRelu_1')(x)

	for i in range(len(argDic['nb_dense_node'])):
		x = res_dense_block_fullpre(inputs = x,
			nb_node = argDic['nb_dense_node'][i],
			initializer = argDic['initializer'],
			regularizer = argDic['wd'],
			drop_ratio = argDic['drop_ratio'],
			name = 'dense_res_block_%d_'%i)
	x = Dropout(argDic['drop_ratio'], name= 'e2e_Dropout_2')(x)
	#x = Activation('relu', name = 'e2e_Relu_1')(x)
	x = LeakyReLU(name = 'e2e_LRelu_2')(x)

	x = Dense(argDic['nb_dense_node'][0],
		kernel_initializer = argDic['initializer'],
		kernel_regularizer = regularizers.l2(argDic['wd']),
		name = 'e2e_dense_3')(x)
	x = Dropout(argDic['drop_ratio'], name= 'e2e_Dropout_3')(x)
	#x = Activation('relu', name = 'e2e_Relu_2')(x)
	x = LeakyReLU(name = 'e2e_LRelu_3')(x)
